...
- `rust-agent-tui/src/app/agent_events_bg.rs:6-201` —— `handle_background_task_completed`，continuation 逻辑仅在 `agent_done_pending_bg == true` 时触发
- `rust-agent-tui/src/app/agent_events_bg.rs:131-134` —— 竞态关键点：检查 `agent_done_pending_bg && background_task_count == 0`
- `rust-agent-tui/src/app/agent_ops.rs:360-373` —— Done 事件处理：`background_task_count > 0` 时设 `agent_done_pending_bg = true`，否则 `agent_rx = None`
- `rust-agent-tui/src/app/agent_ops.rs:835-848` —— `poll_agent` 中消费 `pending_bg_continuation`
- `rust-agent-middlewares/src/subagent/tool.rs:505-557` —— 后台任务 spawn：`tokio::spawn` 中通过 `event_handler.on_event(BackgroundTaskCompleted)` 发送完成通知
- `rust-agent-tui/src/app/agent_ops.rs:906-930` —— `Disconnected` 处理：静默清理后台任务场景
...
**文件**: `rust-agent-tui/src/app/agent_comm.rs`
...
**文件**: `rust-agent-tui/src/ui/headless_test.rs`
...
| 文件 | 变更 |
|------|------|
| `rust-agent-tui/src/app/agent_comm.rs` | 新增 `pre_done_bg_completions` 字段 |
| `rust-agent-tui/src/app/agent_events_bg.rs` | 修改暂存逻辑 + 提取 `build_bg_display_notification()` |
| `rust-agent-tui/src/app/agent_ops.rs` | Done/Error/Disconnected 处理检查暂存 |
| `rust-agent-tui/src/app/agent_submit.rs` | `submit_message` 清理暂存 |
| `rust-agent-tui/src/app/agent_compact.rs` | `handle_compact_done` 清理暂存 |
| `rust-agent-tui/src/ui/headless_test.rs` | 新增 4 个测试 |
