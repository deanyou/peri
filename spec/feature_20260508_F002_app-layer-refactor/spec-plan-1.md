...
- 涉及 rust-agent-tui/src/app/ 下 6 个新文件（service_registry.rs, session_manager.rs, ui_state.rs, message_state.rs, command_system.rs, session_metadata.rs）和 18+ 个现有文件的字段访问路径迁移
...
**背景:**
App 结构体当前 26 字段混合 6 种职责，其中 14 个服务字段（zen_config, cwd, provider_name, model_name, permission_mode, thread_store, mcp_pool, mcp_init_rx, cron, plugin_data, bg_event_tx, bg_event_rx, config_path_override, claude_settings_override）+ 6 个特殊生命周期字段（setup_wizard, oauth_prompt, mode_highlight_until, model_highlight_until, mcp_ready_shown_until, quit_pending_since）属于全局服务/状态，跨 session 共享。本 Task 将这 20 个字段提取到 ServiceRegistry 子结构体，App 通过 `services: ServiceRegistry` 单字段持有。提取后 App 字段数从 26 降至 7（sessions, active, session_areas, services, global_panels + 2 个暂未迁移字段），为 Task 2 SessionManager 提取铺路。本 Task 无前置依赖，其输出被 Task 2-9 全部后续 Task 依赖。

**涉及文件:**
- 新建: `rust-agent-tui/src/app/service_registry.rs`
- 修改: `rust-agent-tui/src/app/mod.rs`, `rust-agent-tui/src/app/panel_ops.rs`, `rust-agent-tui/src/event.rs`, `rust-agent-tui/src/app/agent_ops.rs`, `rust-agent-tui/src/app/thread_ops.rs`, `rust-agent-tui/src/app/cron_ops.rs`, `rust-agent-tui/src/app/message_pipeline.rs`, `rust-agent-tui/src/app/mcp_panel.rs`, `rust-agent-tui/src/app/plugin_panel.rs`, `rust-agent-tui/src/app/panel_manager.rs`, `rust-agent-tui/src/ui/main_ui/status_bar.rs`, `rust-agent-tui/src/ui/main_ui/panels/status.rs`, `rust-agent-tui/src/ui/main_ui/popups/setup_wizard.rs`, `rust-agent-tui/src/ui/main_ui/popups/oauth.rs`, `rust-agent-tui/src/ui/main_ui.rs`, `rust-agent-tui/src/main.rs`, `rust-agent-tui/src/command/model.rs`, `rust-agent-tui/src/command/agents.rs`, `rust-agent-tui/src/ui/welcome.rs`, `rust-agent-tui/src/ui/headless.rs`

**执行步骤:**

- [x] 创建 ServiceRegistry 结构体定义
  - 位置: 新建 `rust-agent-tui/src/app/service_registry.rs`
...
- [x] 在 mod.rs 中注册模块并添加 App.services 字段（双写期）
  - 位置: `rust-agent-tui/src/app/mod.rs` 模块声明区（~L1-36），追加 `mod service_registry;` 和 `pub use service_registry::ServiceRegistry;`
...
- [x] 迁移 cron_ops.rs 中 self.xxx 访问路径
  - 位置: `rust-agent-tui/src/app/cron_ops.rs` 的 `impl App` 块
  - 替换 `self.cron` → `self.services.cron`，`self.zen_config` → `self.services.zen_config`（共 4 处）
  - 原因: cron 操作需要 cron 调度器和 zen_config
...
- [x] 迁移 mcp_panel.rs 中 App 字段访问路径
  - 位置: `rust-agent-tui/src/app/mcp_panel.rs` 的 `impl McpPanel` render 方法
...
- [x] 迁移 plugin_panel.rs 中 self.xxx 访问路径
  - 位置: `rust-agent-tui/src/app/plugin_panel.rs`
...
- [x] 迁移 event.rs 中 app.xxx 访问路径（66 处，分 3 组处理）
  ...
  - 第 1 组——全局弹窗拦截（~L81-434）: 替换 `app.quit_pending_since` → `app.services.quit_pending_since`（~L81,83,423,424,430,434,723），`app.permission_mode` → `app.services.permission_mode`（~L112），`app.mode_highlight_until` → `app.services.mode_highlight_until`（~L113），`app.zen_config` → `app.services.zen_config`（~L124,130），`app.config_path_override` → `app.services.config_path_override`（~L130），`app.provider_name` → `app.services.provider_name`（~L137），`app.model_name` → `app.services.model_name`（~L138），`app.model_highlight_until` → `app.services.model_highlight_until`（~L140），`app.setup_wizard` → `app.services.setup_wizard`（~L188,190,215,733）
  - 第 2 组——PanelContext 构造（~L244-258, 296-308, 757-802 三处重复模式）: 替换 `app.cwd.clone()` → `app.services.cwd.clone()`，`&mut app.zen_config` → `&mut app.services.zen_config`，`app.config_path_override.clone()` → `app.services.config_path_override.clone()`，`app.claude_settings_override.as_ref()` → `app.services.claude_settings_override.as_ref()`，`&mut app.provider_name` → `&mut app.services.provider_name`，`&mut app.model_name` → `&mut app.services.model_name`，`&mut app.mcp_pool` → `&mut app.services.mcp_pool`，`&mut app.cron` → `&mut app.services.cron`，`&mut app.plugin_data` → `&mut app.services.plugin_data`，`&app.bg_event_tx` → `&app.services.bg_event_tx`，`&app.thread_store` → `&app.services.thread_store`
...
- [x] 迁移 UI 层文件中的 app.xxx 访问路径
  - 位置: `rust-agent-tui/src/ui/main_ui/status_bar.rs`（~L122,135,136,140 替换 `app.mcp_init_rx` → `app.services.mcp_init_rx`，`app.mcp_ready_shown_until` → `app.services.mcp_ready_shown_until`）
  - 位置: `rust-agent-tui/src/ui/main_ui/panels/status.rs`（1 处，替换 `app.zen_config` → `app.services.zen_config`）
  - 位置: `rust-agent-tui/src/ui/main_ui/popups/setup_wizard.rs`（2 处，替换 `app.setup_wizard` → `app.services.setup_wizard`）
  - 位置: `rust-agent-tui/src/ui/main_ui/popups/oauth.rs`（3 处，替换 `app.oauth_prompt` → `app.services.oauth_prompt`）
  - 位置: `rust-agent-tui/src/ui/main_ui.rs`（4 处，替换 `app.setup_wizard` / `app.oauth_prompt` / `app.mcp_pool` 等）
  - 位置: `rust-agent-tui/src/ui/welcome.rs`（2 处，替换 `app.provider_name` / `app.model_name`）
  - 原因: UI 渲染层通过 `&mut App` 参数访问服务字段

- [x] 迁移 headless.rs 测试文件中的 app.xxx 访问路径
  - 位置: `rust-agent-tui/src/ui/headless.rs` 测试模块
...
- [x] 迁移 main.rs 和 command/ 模块
  - 位置: `rust-agent-tui/src/main.rs`（~L180,184,186,190,201,287 替换 `app.permission_mode` → `app.services.permission_mode`，`app.zen_config` → `app.services.zen_config`，`app.setup_wizard` → `app.services.setup_wizard`，`app.plugin_data` → `app.services.plugin_data`，`app.mcp_pool` → `app.services.mcp_pool`）
  - 位置: `rust-agent-tui/src/command/model.rs`（~L19,21,28,29 替换 `app.zen_config` → `app.services.zen_config`，`app.config_path_override` → `app.services.config_path_override`，`app.provider_name` → `app.services.provider_name`，`app.model_name` → `app.services.model_name`）
  - 位置: `rust-agent-tui/src/command/agents.rs`（~L21 替换 `app.cwd` → `app.services.cwd`）
  - 原因: main.rs 和 command 模块是 App 外部的消费者

- [x] 删除 App 中 20 个旧字段（结束双写期）
  - 位置: `rust-agent-tui/src/app/mod.rs` 的 App 结构体（~L93-134）
  - 删除 `cwd`, `provider_name`, `model_name`, `zen_config`, `thread_store`, `cron`, `setup_wizard`, `permission_mode`, `mode_highlight_until`, `model_highlight_until`, `config_path_override`, `claude_settings_override`, `mcp_pool`, `mcp_init_rx`, `oauth_prompt`, `bg_event_tx`, `bg_event_rx`, `mcp_ready_shown_until`, `plugin_data`, `quit_pending_since` 共 20 个字段
  - 同步删除 App::new() 中对应的旧字段赋值
  - 原因: 双写期结束后清理旧字段，App 从 26 字段降至 7 字段

- [x] 为 ServiceRegistry 编写单元测试
  - 测试文件: `rust-agent-tui/src/app/service_registry.rs` 底部 `#[cfg(test)] mod tests`
  - 测试场景:
    ...
    - `test_app_no_old_fields`: 编译期验证——尝试访问 `app.zen_config` 应产生编译错误（通过 `#[cfg(test)]` 注释代码标记）
  - 运行命令: `cargo test -p rust-agent-tui --lib -- service_registry`
  - 预期: 所有测试通过

**检查步骤:**
- [x] 验证 App 旧字段已删除
  - `grep -n "pub cwd:" rust-agent-tui/src/app/mod.rs | head -5`
...
- [x] 验证无残留的 app.xxx 旧路径访问
  - `grep -rn "app\.zen_config\|app\.cwd\b\|app\.provider_name\|app\.model_name\|app\.permission_mode\|app\.thread_store\|app\.mcp_pool\|app\.mcp_init_rx\|app\.cron\b\|app\.plugin_data\|app\.bg_event_tx\|app\.bg_event_rx\|app\.config_path_override\|app\.claude_settings_override\|app\.setup_wizard\|app\.oauth_prompt\|app\.mode_highlight\|app\.model_highlight\|app\.mcp_ready_shown\|app\.quit_pending" rust-agent-tui/src/ | grep -v "app\.services\." | grep -v "spec-plan"`
  - 预期: 无匹配结果（所有旧路径已迁移到 app.services.xxx）
- [x] 验证无残留的 self.xxx 旧路径访问（app/ 目录内）
  - `grep -rn "self\.zen_config\|self\.cwd\b\|self\.provider_name\|self\.model_name\|self\.permission_mode\|self\.thread_store\|self\.mcp_pool\|self\.mcp_init_rx\|self\.cron\b\|self\.plugin_data\|self\.bg_event_tx\|self\.bg_event_rx\|self\.config_path_override\|self\.claude_settings_override\|self\.setup_wizard\|self\.oauth_prompt\|self\.mode_highlight\|self\.model_highlight\|self\.mcp_ready_shown\|self\.quit_pending" rust-agent-tui/src/app/ | grep -v "self\.services\."`
  - 预期: 无匹配结果
- [x] 验证编译通过
  - `cargo build -p rust-agent-tui 2>&1 | tail -5`
  - 预期: 编译成功，无错误
- [x] 验证 headless 测试通过
  - `cargo test -p rust-agent-tui --lib -- headless 2>&1 | tail -10`
  - 预期: 所有 headless 测试通过
- [x] 验证 ServiceRegistry 单元测试通过
  - `cargo test -p rust-agent-tui --lib -- service_registry 2>&1 | tail -10`
...
**涉及文件:**
- 新建: `rust-agent-tui/src/app/session_manager.rs`
- 修改: `rust-agent-tui/src/app/mod.rs`, `rust-agent-tui/src/event.rs`, `rust-agent-tui/src/ui/main_ui.rs`, `rust-agent-tui/src/ui/headless.rs`, `rust-agent-tui/src/main.rs`, `rust-agent-tui/src/app/agent_ops.rs`, `rust-agent-tui/src/app/hint_ops.rs`, `rust-agent-tui/src/app/panel_ops.rs`, `rust-agent-tui/src/command/model.rs`, `rust-agent-tui/src/command/agent.rs`, `rust-agent-tui/src/command/help.rs`, `rust-agent-tui/src/command/history.rs`, `rust-agent-tui/src/command/loop_cmd.rs`, `rust-agent-tui/src/command/compact.rs`, `rust-agent-tui/src/ui/welcome.rs`, `rust-agent-tui/src/ui/main_ui/popups/hints.rs`, `rust-agent-tui/src/ui/main_ui/popups/hitl.rs`, `rust-agent-tui/src/ui/main_ui/popups/ask_user.rs`, `rust-agent-tui/src/ui/main_ui/status_bar.rs`, `rust-agent-tui/src/ui/main_ui/sticky_header.rs`, `rust-agent-tui/src/ui/main_ui/panels/mcp.rs`, `rust-agent-tui/src/ui/main_ui/panels/agent.rs`, `rust-agent-tui/src/ui/main_ui/panels/hooks.rs`, `rust-agent-tui/src/ui/main_ui/panels/status.rs`, `rust-agent-tui/src/ui/main_ui/panels/thread_browser.rs`, `rust-agent-tui/src/ui/main_ui/panels/cron.rs`, `rust-agent-tui/src/ui/main_ui/panels/plugin.rs`, `rust-agent-tui/src/ui/main_ui/panels/model.rs`, `rust-agent-tui/src/ui/main_ui/panels/memory.rs`, `rust-agent-tui/src/ui/main_ui/panels/login.rs`

**执行步骤:**
- [x] 创建 `SessionManager` 结构体，定义字段 + 辅助方法
  - 位置: 新建 `rust-agent-tui/src/app/session_manager.rs`
...
- [x] 在 `app/mod.rs` 中注册模块、添加 `session_mgr` 字段（双写期）
  - 位置: `rust-agent-tui/src/app/mod.rs` — 文件顶部 `mod` 声明区（~L18 后）追加 `mod session_manager;`
  - 位置: `rust-agent-tui/src/app/mod.rs` — `App` 结构体（~L93），在 `session_areas` 字段后追加:
    ```rust
    pub session_mgr: session_manager::SessionManager,
    ```
  - 位置: `rust-agent-tui/src/app/mod.rs` — `App::new()` 返回值（~L198），在 `Self { ... }` 中追加:
    ```rust
  ...
  - 位置: `rust-agent-tui/src/app/mod.rs` — re-export 区（~L80）追加:
    ```rust
...
- [x] 迁移 `app/mod.rs` 中 `App` 的 session 管理方法
  - 位置: `rust-agent-tui/src/app/mod.rs` — `active()` (~L231), `active_mut()` (~L236), `session_at()` (~L241), `session_at_mut()` (~L246)
  - 改为委托到 `session_mgr`:
    ```rust
    ...
    ```
  - 位置: `rust-agent-tui/src/app/mod.rs` — `new_session()` (~L251), `close_session()` (~L284), `switch_next_session()` (~L302), `switch_prev_session()` (~L309)
  - 将这些方法内部的 `self.sessions` / `self.active` 全部替换为 `self.session_mgr.sessions` / `self.session_mgr.active`，保持旧字段同步（双写期）
  - 位置: `rust-agent-tui/src/app/mod.rs` — `interrupt()` (~L380) 中所有 `self.sessions[self.active]` 替换为 `self.session_mgr.sessions[self.session_mgr.active]`，旧字段同步写入
  - 原因: 方法迁移先行，确保 `App` 的公共 API 通过委托保持一致，外部调用者无需改动

- [x] 迁移 `agent_ops.rs` 中 31 处 `self.sessions[self.active]` 访问
  - 位置: `rust-agent-tui/src/app/agent_ops.rs` — 全文件
...
- [x] 迁移 `hint_ops.rs` 中 28 处 `self.sessions[self.active]` 访问
  - 位置: `rust-agent-tui/src/app/hint_ops.rs` — `impl App` 内全部方法
...
- [x] 迁移 `event.rs` 中 118 处 `app.sessions` / `app.active` 访问
  - 位置: `rust-agent-tui/src/event.rs` — 全文件
  - 替换规则:
...
- [x] 迁移 `main_ui.rs` 中 48 处 `app.sessions` / `app.active` 访问，消除临时 active 交换 workaround
  - 位置: `rust-agent-tui/src/ui/main_ui.rs` — `render_session_column()` (~L63)
  ...
  - 位置: `rust-agent-tui/src/ui/main_ui.rs` — `render_messages()` (~L335), `render_attachment_bar()` (~L559), `active_panel_height()` (~L270)
  ...
  - 位置: `rust-agent-tui/src/ui/main_ui.rs` — `render()` (~L19) 中 `app.session_areas = cols...` 改为 `app.session_mgr.session_areas = cols...`
  - 原因: 消除 `main_ui.rs` 中最危险的 workaround（临时切换全局 `active` 索引），这是本 Task 的重要设计目标

- [x] 迁移 `headless.rs` 中 109 处 `app.sessions` / `app.active` 访问
  - 位置: `rust-agent-tui/src/ui/headless.rs` — 测试函数中所有 `app.sessions[app.active]`
...
- [x] 迁移 `main.rs` 中 10 处 `app.sessions` / `app.active` 访问
  - 位置: `rust-agent-tui/src/main.rs` — 主循环中 spinner tick、render poll 等
...
- [x] 迁移 `panel_ops.rs` 中 `new_headless()` 的字段初始化
  - 位置: `rust-agent-tui/src/app/panel_ops.rs` — `new_headless()` (~L1088)
...
- [x] 迁移 command/ 目录下 6 个文件中的 `app.sessions[app.active]` 访问
  - 位置: `rust-agent-tui/src/command/model.rs`, `agent.rs`, `help.rs`, `history.rs`, `loop_cmd.rs`, `compact.rs`
...
- [x] 迁移 ui/main_ui/panels/ 下 11 个面板文件中的 `app.sessions[app.active]` 访问
  - 位置: `rust-agent-tui/src/ui/main_ui/panels/` 下所有 `*.rs` 文件
...
- [x] 迁移 ui/main_ui/popups/ 和 ui/main_ui/status_bar.rs, sticky_header.rs, welcome.rs
  - 位置: `rust-agent-tui/src/ui/main_ui/popups/{hints,hitl,ask_user}.rs`, `status_bar.rs`, `sticky_header.rs`, `ui/welcome.rs`
  - 替换规则同上
  - 原因: 覆盖所有使用 `app.sessions[app.active]` 的 UI 文件

- [x] 删除 App 中旧字段 `sessions`, `active`, `session_areas`
  - 位置: `rust-agent-tui/src/app/mod.rs` — `App` 结构体定义（~L95-L99）
...
- [x] 为 SessionManager 核心逻辑编写单元测试
  - 测试文件: `rust-agent-tui/src/app/session_manager.rs`（模块内 `#[cfg(test)] mod tests`）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-tui --lib -- session_manager`
  - 预期: 所有测试通过
...
- [x] 验证无残留旧路径访问（不含 session_mgr 前缀）
  - `grep -rn 'app\.sessions\b' rust-agent-tui/src/ | grep -v 'session_mgr' | grep -v '//.*app\.sessions' | wc -l`
  - 预期: 输出 0
- [x] 验证无残留 `app.active` 直接赋值（不含 session_mgr 前缀）
  - `grep -rn 'app\.active\s*=' rust-agent-tui/src/ | grep -v 'session_mgr' | wc -l`
  - 预期: 输出 0
- [x] 验证无残留 `app.session_areas` 直接访问
  - `grep -rn 'app\.session_areas' rust-agent-tui/src/ | grep -v 'session_mgr' | wc -l`
  - 预期: 输出 0
- [x] 验证 SessionManager 单元测试通过
  - `cargo test -p rust-agent-tui --lib -- session_manager`
  - 预期: 所有测试通过
- [x] 验证 headless 测试通过
  - `cargo test -p rust-agent-tui --lib -- ui::headless`
  - 预期: 所有测试通过
- [x] 验证 clippy 无警告
  - `cargo clippy -p rust-agent-tui 2>&1 | grep -E 'warning|error' | head -10`
  - 预期: 无新增 warning
...
**涉及文件:**
- 新建: `rust-agent-tui/src/app/ui_state.rs`
- 修改: `rust-agent-tui/src/app/core.rs`, `rust-agent-tui/src/app/chat_session.rs`, `rust-agent-tui/src/app/mod.rs`, `rust-agent-tui/src/event.rs`, `rust-agent-tui/src/ui/main_ui.rs`, `rust-agent-tui/src/ui/headless.rs`, `rust-agent-tui/src/app/panel_ops.rs`, `rust-agent-tui/src/app/agent_ops.rs`, `rust-agent-tui/src/app/hint_ops.rs`, `rust-agent-tui/src/app/history_ops.rs`, `rust-agent-tui/src/app/thread_ops.rs`, `rust-agent-tui/src/app/mcp_panel.rs`, `rust-agent-tui/src/app/agent_panel.rs`, `rust-agent-tui/src/app/hooks_panel.rs`, `rust-agent-tui/src/app/plugin_panel.rs`, `rust-agent-tui/src/app/ask_user_ops.rs`, `rust-agent-tui/src/app/cron_ops.rs`, `rust-agent-tui/src/command/history.rs`, `rust-agent-tui/src/command/plugin_command.rs`, `rust-agent-tui/src/command/compact.rs`, `rust-agent-tui/src/main.rs`, `rust-agent-tui/src/ui/main_ui/status_bar.rs`, `rust-agent-tui/src/ui/main_ui/popups/hints.rs`, `rust-agent-tui/src/ui/main_ui/popups/ask_user.rs`, `rust-agent-tui/src/ui/main_ui/panels/mcp.rs`, `rust-agent-tui/src/ui/main_ui/panels/agent.rs`, `rust-agent-tui/src/ui/main_ui/panels/hooks.rs`, `rust-agent-tui/src/ui/main_ui/panels/memory.rs`, `rust-agent-tui/src/ui/main_ui/panels/thread_browser.rs`, `rust-agent-tui/src/ui/main_ui/panels/cron.rs`, `rust-agent-tui/src/ui/main_ui/panels/plugin.rs`, `rust-agent-tui/src/ui/render_thread.rs`

**执行步骤:**

- [x] 创建 `rust-agent-tui/src/app/ui_state.rs`，定义 UiState 结构体及构造方法
  - 位置: 新建 `rust-agent-tui/src/app/ui_state.rs`
...
- [x] 在 `rust-agent-tui/src/app/mod.rs` 中注册模块
  - 位置: `rust-agent-tui/src/app/mod.rs` 模块声明区域（~L15 后），在 `pub mod text_selection;` 之后
  - 添加: `pub mod ui_state;`
  - 原因: ui_state.rs 需作为 app 子模块可见

- [x] 在 ChatSession 中新增 `ui: UiState` 字段（双写阶段）
  - 位置: `rust-agent-tui/src/app/chat_session.rs` ChatSession 结构体（~L11）
...
- [x] 在 `panel_ops.rs` 的 `new_headless()` 中初始化 `ui` 字段
  - 位置: `rust-agent-tui/src/app/panel_ops.rs` ChatSession 构造（~L1076）
...
- [x] 迁移 `main_ui.rs` 中 16 处 UiState 字段访问
  - 位置: `rust-agent-tui/src/ui/main_ui.rs` 全文件
...
- [x] 迁移 `headless.rs` 中 UiState 字段访问
  - 位置: `rust-agent-tui/src/ui/headless.rs` 测试函数
...
- [x] 迁移 `panel_ops.rs` 中 8 处 UiState 字段访问
  - 位置: `rust-agent-tui/src/app/panel_ops.rs` 全文件
...
- [x] 迁移 command/ 子模块和 main.rs 中 UiState 字段访问
  - 位置:
    - `rust-agent-tui/src/command/history.rs`（1 处）: `core.textarea` → `ui.textarea`
    - `rust-agent-tui/src/command/plugin_command.rs`（1 处）: `core.textarea` → `ui.textarea`
    - `rust-agent-tui/src/command/compact.rs`（1 处）: `core.textarea` → `ui.textarea`
    - `rust-agent-tui/src/main.rs`（1 处）: `core.textarea` → `ui.textarea`
  - 原因: command 子模块和 main.rs 中少量直接操作 textarea

- [x] 迁移 ui/main_ui/ 子目录中 UiState 字段访问（status_bar, popups, panels）
  - 位置:
    - `rust-agent-tui/src/ui/main_ui/status_bar.rs`（3 处）: `core.loading` / `core.show_tool_messages` → `ui.` 前缀
    - `rust-agent-tui/src/ui/main_ui/popups/hints.rs`（3 处）: `core.hint_cursor` / `core.textarea` → `ui.` 前缀
    - `rust-agent-tui/src/ui/main_ui/popups/ask_user.rs`（1 处）: `core.textarea` → `ui.textarea`
    - `rust-agent-tui/src/ui/main_ui/panels/mcp.rs`（10 处）: `core.panel_selection` / `core.panel_area` / `core.panel_plain_lines` / `core.panel_scroll_offset` → `ui.` 前缀
    - `rust-agent-tui/src/ui/main_ui/panels/agent.rs`（6 处）: 同上 → `ui.` 前缀
    - `rust-agent-tui/src/ui/main_ui/panels/hooks.rs`（4 处）: 同上 → `ui.` 前缀
    - `rust-agent-tui/src/ui/main_ui/panels/memory.rs`（4 处）: 同上 → `ui.` 前缀
    - `rust-agent-tui/src/ui/main_ui/panels/thread_browser.rs`（8 处）: 同上 → `ui.` 前缀
    - `rust-agent-tui/src/ui/main_ui/panels/cron.rs`（7 处）: 同上 → `ui.` 前缀
    - `rust-agent-tui/src/ui/main_ui/panels/plugin.rs`（17 处）: 同上 → `ui.` 前缀
  - 原因: UI 渲染子模块中对面板选区和 textarea 的访问需统一迁移

- [x] 检查并迁移 `render_thread.rs` 中可能存在的 UiState 字段访问
  - 位置: `rust-agent-tui/src/ui/render_thread.rs`（1 处）
...
- [x] 为 UiState 编写单元测试
  - 测试文件: `rust-agent-tui/src/app/ui_state.rs`（模块内 `#[cfg(test)] mod tests`）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-tui --lib -- ui_state::tests`
  - 预期: 所有测试通过

**检查步骤:**
- [x] 验证 UiState 结构体存在且包含 18 个字段
  - `grep -c "pub " rust-agent-tui/src/app/ui_state.rs`
  - 预期: 大于等于 19（18 个字段 + 1 个 new 方法）
- [x] 验证 AppCore 不再包含 UiState 字段
  - `grep -E "pub (textarea|loading|scroll_offset|scroll_follow|show_tool_messages|hint_cursor|input_history|history_index|draft_input|text_selection|messages_area|textarea_area|copy_message_until|copy_char_count|panel_selection|panel_area|panel_plain_lines|panel_scroll_offset)" rust-agent-tui/src/app/core.rs`
  - 预期: 0 行输出
- [x] 验证 event.rs 中无 `core.` 前缀的 UiState 字段访问
  - `grep -E "core\.(textarea|loading|scroll_offset|scroll_follow|show_tool_messages|hint_cursor|input_history|history_index|draft_input|text_selection|messages_area|textarea_area|copy_message_until|copy_char_count|panel_selection|panel_area|panel_plain_lines|panel_scroll_offset)" rust-agent-tui/src/event.rs`
  - 预期: 0 行输出
- [x] 验证全项目无残留的 UiState 字段通过 core 访问
  - `grep -rn "core\.\(textarea\|loading\|scroll_offset\|scroll_follow\|show_tool_messages\|hint_cursor\|input_history\|history_index\|draft_input\|text_selection\|messages_area\|textarea_area\|copy_message_until\|copy_char_count\|panel_selection\|panel_area\|panel_plain_lines\|panel_scroll_offset\)" rust-agent-tui/src/`
  - 预期: 0 行输出
- [x] 验证编译通过
  - `cargo build -p rust-agent-tui 2>&1 | tail -5`
  - 预期: 构建成功，无错误
...
- [x] 验证全部 headless 测试通过
  ...
  - 预期: 所有测试通过
- [x] 验证 UiState 单元测试通过
  - `cargo test -p rust-agent-tui --lib -- ui_state::tests 2>&1 | tail -10`
  - 预期: 所有测试通过
...
**涉及文件:**
- 新建: `rust-agent-tui/src/app/message_state.rs`
- 修改: `rust-agent-tui/src/app/core.rs`, `rust-agent-tui/src/app/chat_session.rs`, `rust-agent-tui/src/app/mod.rs`, `rust-agent-tui/src/app/agent_ops.rs`, `rust-agent-tui/src/app/thread_ops.rs`, `rust-agent-tui/src/app/panel_ops.rs`, `rust-agent-tui/src/event.rs`, `rust-agent-tui/src/ui/main_ui.rs`, `rust-agent-tui/src/ui/headless.rs`, `rust-agent-tui/src/main.rs`, `rust-agent-tui/src/command/loop_cmd.rs`, `rust-agent-tui/src/command/agent.rs`, `rust-agent-tui/src/command/help.rs`, `rust-agent-tui/src/app/login_panel.rs`, `rust-agent-tui/src/app/plugin_panel.rs`, `rust-agent-tui/src/app/agent_panel.rs`, `rust-agent-tui/src/app/config_panel.rs`, `rust-agent-tui/src/app/cron_state.rs`, `rust-agent-tui/src/app/cron_ops.rs`, `rust-agent-tui/src/thread/browser.rs`

**执行步骤:**

- [x] 创建 `rust-agent-tui/src/app/message_state.rs`，定义 MessageState 结构体及构造方法
  - 位置: 新建 `rust-agent-tui/src/app/message_state.rs`
...
- [x] 在 `rust-agent-tui/src/app/mod.rs` 中注册模块
  - 位置: `rust-agent-tui/src/app/mod.rs` 模块声明区域，在 `pub mod ui_state;` 之后
...
- [x] 在 ChatSession 中新增 `messages: MessageState` 字段（双写阶段）
  ...
  - 位置: `rust-agent-tui/src/app/chat_session.rs` ChatSession::new()（~L23）
...
- [x] 迁移 `app/mod.rs` 中 5 处 MessageState 字段访问
  - 位置: `rust-agent-tui/src/app/mod.rs` 全文件
...
- [x] 迁移 `thread_ops.rs` 中 9 处 MessageState 字段访问
  - 位置: `rust-agent-tui/src/app/thread_ops.rs` 全文件
  - 替换:
...
- [x] 迁移 `main.rs` 中 2 处 MessageState 字段访问
  - 位置: `rust-agent-tui/src/main.rs`（~L272, 274）
...
- [x] 迁移 command/ 目录下 3 个文件中的 MessageState 字段访问
  - 位置:
    - `rust-agent-tui/src/command/loop_cmd.rs`（~L24, 27, 59, 60, 73, 74, 81, 86, 90）: `core.view_messages` → `messages.view_messages`，`core.render_tx` → `messages.render_tx`
    - `rust-agent-tui/src/command/agent.rs`（~L47, 50）: `core.view_messages` → `messages.view_messages`，`core.render_tx` → `messages.render_tx`
    - `rust-agent-tui/src/command/help.rs`（~L20, 26）: `core.view_messages` → `messages.view_messages`
  - 原因: command 文件中推送系统消息到 view_messages 或发送渲染事件

- [x] 迁移 app/ 子模块中 6 个文件的 MessageState 字段访问
  - 位置:
    - `rust-agent-tui/src/app/login_panel.rs`（6 处，~L430, 441, 562, 578, 588, 627, 638）: `ctx.sessions[ctx.active].core.view_messages` → `ctx.sessions[ctx.active].messages.view_messages`
    - `rust-agent-tui/src/app/plugin_panel.rs`（3 处，~L129, 136, 162）: 同上替换
    - `rust-agent-tui/src/app/agent_panel.rs`（1 处，~L160）: `ctx.sessions[ctx.active].core.view_messages` → `ctx.sessions[ctx.active].messages.view_messages`
    - `rust-agent-tui/src/app/config_panel.rs`（2 处，~L331, 338）: 同上替换
    - `rust-agent-tui/src/app/cron_state.rs`（1 处，~L1184）: `ctx.sessions[ctx.active].core.view_messages` → `ctx.sessions[ctx.active].messages.view_messages`（此处通过 `.view_messages` 访问，需确认完整路径）
    - `rust-agent-tui/src/app/cron_ops.rs`（1 处，~L49）: `core.view_messages` → `messages.view_messages`
  - 原因: 面板和 cron 模块中向 view_messages 推送系统消息

- [x] 迁移 `thread/browser.rs` 中 1 处 MessageState 字段访问
  - 位置: `rust-agent-tui/src/thread/browser.rs`（~L160）
...
- [x] 为 MessageState 编写单元测试
  - 测试文件: `rust-agent-tui/src/app/message_state.rs`（模块内 `#[cfg(test)] mod tests`）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-tui --lib -- message_state::tests`
  - 预期: 所有测试通过

**检查步骤:**
- [x] 验证 MessageState 结构体存在且包含 9 个字段
  - `grep -c "pub " rust-agent-tui/src/app/message_state.rs`
  - 预期: 大于等于 10（9 个字段 + 1 个 new 方法）
- [x] 验证 AppCore 不再包含 MessageState 字段
  - `grep -E "pub (view_messages|round_start_vm_idx|pipeline|render_tx|render_cache|render_notify|last_render_version|pending_messages|last_submitted_text)" rust-agent-tui/src/app/core.rs`
  - 预期: 0 行输出
- [x] 验证 AppCore::new() 不再接受 render 相关参数
  - `grep -E "render_tx|render_cache|render_notify" rust-agent-tui/src/app/core.rs`
  - 预期: 0 行输出
- [x] 验证全项目无残留的 MessageState 字段通过 core 访问
  - `grep -rn "core\.\(view_messages\|round_start_vm_idx\|pipeline\|render_tx\|render_cache\|render_notify\|last_render_version\|pending_messages\|last_submitted_text\)" rust-agent-tui/src/ | grep -v "spec-plan"`
  - 预期: 0 行输出
- [x] 验证编译通过
  - `cargo build -p rust-agent-tui 2>&1 | tail -5`
  - 预期: 构建成功，无错误
...
- [x] 验证 MessageState 单元测试通过
  - `cargo test -p rust-agent-tui --lib -- message_state::tests 2>&1 | tail -10`
  - 预期: 所有测试通过
- [x] 验证 AppCore 现有测试仍通过（core.rs 中 test_appcore_pipeline_initialized 需删除或改写）
  - `cargo test -p rust-agent-tui --lib -- core::tests 2>&1 | tail -10`
...
2. 验证 App 字段数已减少
   - `grep -c "pub [a-z_]*:" rust-agent-tui/src/app/mod.rs | head -1`（粗略检查）
...
4. 验证无旧路径残留（ServiceRegistry 字段）
   - `grep -r "app\.cwd\b" rust-agent-tui/src/ --include="*.rs" | grep -v "services\.cwd" | grep -v "test" | wc -l`
   - 预期: 0（所有 app.cwd 已迁移为 app.services.cwd）

5. 验证 clippy 无警告
   - `cargo clippy -p rust-agent-tui 2>&1 | grep "warning:" | wc -l`
   - 预期: 0

6. 验证 headless 测试通过
   - `cargo test -p rust-agent-tui --lib -- headless 2>&1 | grep "test result"`
   - 预期: 全部通过
