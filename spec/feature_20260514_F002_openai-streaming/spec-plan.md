...
- `rust-create-agent/src/llm/types.rs` -- 新增 `StreamingContext`
- `rust-create-agent/src/llm/mod.rs` -- `BaseModel` trait 新增 `invoke_streaming()` 默认方法
- `rust-create-agent/src/agent/react.rs` -- `Reasoning` 新增 `streamed` 字段 + `ReactLLM` trait 签名变更 + blanket impl 变更
...
```bash
cargo build -p rust-create-agent 2>&1 | head -50
```
...
**文件:** `rust-create-agent/src/llm/sse.rs`（新增）
...
```bash
cargo test -p rust-create-agent --lib -- sse
```
...
**文件:** `rust-create-agent/src/llm/openai.rs`
...
### 5.5 SubAgent 路径守卫（`rust-agent-middlewares/` 不调用处）
...
**文件:** `rust-create-agent/src/llm/openai_test.rs`（新增）
...
```bash
cargo test -p rust-create-agent --lib -- openai_test
```
...
**文件:** `rust-create-agent/src/llm/anthropic.rs` tests 模块（新增）
...
```bash
cargo test -p rust-create-agent --lib -- anthropic
```
...
cargo test -p rust-create-agent --lib -- executor
```
...
```bash
# 全量测试
cargo test -p rust-create-agent

# Clippy
cargo clippy -p rust-create-agent
...
| `rust-create-agent` 中 reqwest 已有 `stream` feature | `Cargo.toml` 已声明，无需新增依赖 |
